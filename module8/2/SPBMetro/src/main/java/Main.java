...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    errorsLogger = LogManager.getLogger("errorsLogger");
    ...
    try {
      ...
      exceptionsLogger.fatal(e.getMessage());
    }
  }
  ...
  private static Station takeStation(String message) {
    for (; ; ) {
      ...
      if (station != null) {
        if (message.split(" ")[2].equals("отправления:")) {
          logger.info("Станция отправления: " + line);
        } else {
          logger.info("Станция назначения: " + line);
        }
        return station;
      }
      errorsLogger.error("Ошибочный ввод: " + line);
